...
class AnimeCharacterSelectorTests(unittest.TestCase):
    ...
    def test_representative_revised_game_mappings(self) -> None:
        expected = {
            ...
            "小豆泽心羽（Project SEKAI）": "azusawa kohane",
            "拉欧拉·潘特拉（Hololive）": "raora panthera",
            "高咲侑（Love Live! 虹咲学园学园偶像同好会）": "takasaki yu",
            "宵崎奏（Project SEKAI）": "yoisaki kanade",
            "小丑女（哈莉·奎茵）（DC）": "harley quinn",
            "日向翔阳（排球少年!!）": "hinata shouyou",
            "杰维斯（舰队Collection）": "jervis (kancolle)",
            "镜音铃（Vocaloid）": "kagamine rin",
            "镜音铃-Append（Vocaloid）": "kagamine rin (append)",
            "赤焰松（宝可梦）": "maxie (pokemon)",
            "松叶（宝可梦）": "morty (pokemon)",
            "温迪（风神形态）（原神）": "venti (archon) (genshin impact)",
            "温迪（原神）": "venti (genshin impact)",
            "白心（超次元游戏海王星）": "white heart (neptunia)",
            "白莲（月姬）": "white len (tsukihime)",
            "紫藤伊莉娜（恶魔高校D×D）": "shidou irina",
            "东条斩美（弹丸论破V3）": "tojo kirumi",
            "猫鼬斩（宝可梦）": "zangoose",
